...
CONFIG_PATH="${PROJECT_ROOT}/configs/tune_baseline4.json"
...
if [[ "${QUICK}" -eq 1 && "${CUSTOM_CONFIG}" -eq 0 ]]; then
  CONFIG_PATH="${PROJECT_ROOT}/configs/tune_baseline4_quick.json"
fi
...
echo "[baseline4-retune] config=${CONFIG_PATH}"
echo "[baseline4-retune] families=${FAMILIES}"
echo "[baseline4-retune] command=${CMD[*]} ${EXTRA_ARGS[*]}"

"${CMD[@]}" "${EXTRA_ARGS[@]}"

echo "[baseline4-retune] done"
